      ...
      await transporter.sendMail({
        ...
        html: `
          <div style="font-family:Arial, sans-serif; padding:20px;">
            ...
            <p>Hello Team, please find the details below:</p>
...
exports.submitQuotation = async (req, res) => {
  try {
    ...
    // Database mein wahi record dhoondo jiska email match karta ho aur update karo
    const updatedQuotation = await Quotation.findOneAndUpdate(
      { clientEmail: clientEmail }, // Filter: is email wala enquiry dhoondo
      { 
        amount: amount, 
        description: description, 
        status: "Quoted" 
      },
      { new: true } // Taaki response mein updated data mile
    );
    ...
    res.json({ success: true, message: "Quotation Updated!", data: updatedQuotation });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
};
